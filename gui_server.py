...
class MetricsHandler(SimpleHTTPRequestHandler):
    ...
    def build_preview_payload(self, row, include_previews=True, car_only_enabled=False):
        payload = {
            ...
            "mask_metric_scope": row.get("mask_metric_scope", "car_mask" if row.get("mask_iou") else "none"),
            "mask_metrics_available": row.get("mask_metric_scope") == "car_mask" or bool(row.get("mask_iou")),
    ...
    def do_POST(self):
        ...
        try:
            ...
            if is_no_space_error(exc):
                message = get_no_space_message()
            else:
                message = str(exc)
            print(f"[API] Vergleich fehlgeschlagen: {message}")
    ...
    def run_image_metrics(self, payload):
        ...
        if process.returncode != 0:
            process_output = process.stderr.strip() or process.stdout.strip()
            if is_no_space_error(RuntimeError(process_output)):
